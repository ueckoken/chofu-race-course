...
var _Horse_Name_Pattern = regexp.MustCompile("^\\p{Katakana}(\\p{Katakana}|ー){1,8}$")
...
// Validate checks the field values on RaceDetails with the rules defined in
// the proto definition for this message. If any rules are violated, the first
// error encountered is returned, or nil if there are no violations.
func (m *RaceDetails) Validate() error {
	return m.validate(false)
}

// ValidateAll checks the field values on RaceDetails with the rules defined in
// the proto definition for this message. If any rules are violated, the
// result is a list of violation errors wrapped in RaceDetailsMultiError, or
// nil if none found.
func (m *RaceDetails) ValidateAll() error {
	return m.validate(true)
}

func (m *RaceDetails) validate(all bool) error {
	if m == nil {
		return nil
	}
	...
	for idx, item := range m.GetRaceDetails() {
		_, _ = idx, item
		...
		if all {
			...
			case interface{ ValidateAll() error }:
				if err := v.ValidateAll(); err != nil {
					errors = append(errors, RaceDetailsValidationError{
						field:  fmt.Sprintf("RaceDetails[%v]", idx),
						reason: "embedded message failed validation",
						cause:  err,
					})
				}
			...
			if err := v.Validate(); err != nil {
				return RaceDetailsValidationError{
					field:  fmt.Sprintf("RaceDetails[%v]", idx),
					reason: "embedded message failed validation",
					cause:  err,
				}
	...
	if len(errors) > 0 {
		return RaceDetailsMultiError(errors)
	}

	return nil
}

// RaceDetailsMultiError is an error wrapping multiple validation errors
// returned by RaceDetails.ValidateAll() if the designated constraints aren't met.
type RaceDetailsMultiError []error

// Error returns a concatenation of all the error messages it wraps.
func (m RaceDetailsMultiError) Error() string {
	var msgs []string
	for _, err := range m {
		msgs = append(msgs, err.Error())
	}
...
// AllErrors returns a list of validation violation errors.
func (m RaceDetailsMultiError) AllErrors() []error { return m }

// RaceDetailsValidationError is the validation error returned by
// RaceDetails.Validate if the designated constraints aren't met.
type RaceDetailsValidationError struct {
	field  string
	reason string
	cause  error
	key    bool
...
// Field function returns field value.
func (e RaceDetailsValidationError) Field() string { return e.field }

// Reason function returns reason value.
func (e RaceDetailsValidationError) Reason() string { return e.reason }

// Cause function returns cause value.
func (e RaceDetailsValidationError) Cause() error { return e.cause }

// Key function returns key value.
func (e RaceDetailsValidationError) Key() bool { return e.key }

// ErrorName returns error name.
func (e RaceDetailsValidationError) ErrorName() string { return "RaceDetailsValidationError" }

// Error satisfies the builtin error interface
func (e RaceDetailsValidationError) Error() string {
	cause := ""
	if e.cause != nil {
		cause = fmt.Sprintf(" | caused by: %v", e.cause)
	}
...
var _ error = RaceDetailsValidationError{}
